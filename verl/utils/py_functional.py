...
def union_two_dict(dict1: Dict[str, Any], dict2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in dict2.keys():
        if key in dict1:
            left, right = dict1[key], dict2[key]
            if isinstance(left, torch.Tensor) or isinstance(right, torch.Tensor):
                same_value = isinstance(left, torch.Tensor) and isinstance(right, torch.Tensor) and torch.equal(left, right)
            elif isinstance(left, np.ndarray) or isinstance(right, np.ndarray):
                same_value = isinstance(left, np.ndarray) and isinstance(right, np.ndarray) and np.array_equal(left, right)
            else:
