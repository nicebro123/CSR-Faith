...
class AlgorithmConfig:
    ...
    # ── CIT-Faith: Faithfulness Optimization ──
    enable_citfaith: bool = False
    # Reviewer model config
    reviewer_model_path: str = "Qwen/Qwen2.5-7B-Instruct-AWQ"
    reviewer_gpu_memory: float = 0.15
    # Lagrangian dual constraint thresholds
    tau_sc: float = 0.8          # Self-consistency threshold
    tau_pr: float = 0.7          # Perception-thinking coherence threshold
    dual_lr: float = 0.01       # Lagrangian multiplier learning rate (eta)
    # Causal modulation
    cfs_alpha: float = 0.1      # Lower bound for modulation g(r) = alpha + (1-alpha)*r
    # Counterfactual continuation
    cf_max_tokens: int = 50     # Max tokens for counterfactual answer generation

    # ── CSR-Faith: Causal Spatial Rationales ──
