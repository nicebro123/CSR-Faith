...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if self.generation_config.pad_token_id is None:
            self.generation_config.pad_token_id = self._pad_token_id()
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def generate_continuations(self, data: DataProto) -> DataProto:
        """
        CIT-Faith: Prefix-continuation decode for counterfactual intervention.
        """
        assert self._is_rollout
        ...
        prefix_ids = data.batch["prefix_ids"]
        prefix_mask = data.batch["prefix_mask"]
        multi_modal_data = data.non_tensor_batch.get("multi_modal_data")
        cf_max_tokens = data.meta_info.get("cf_max_tokens", 50)
        batch_size = prefix_ids.shape[0]

        # Extract actual token ids (remove padding)
        prefix_ids_list = []
        for i in range(batch_size):
            valid_len = prefix_mask[i].sum().int().item()
            valid_ids = prefix_ids[i, :valid_len].tolist()
            prefix_ids_list.append(valid_ids)

        with self.rollout_sharding_manager:
            # After weight sync, offload FSDP model to free GPU memory
            if self._use_param_offload:
                offload_fsdp_model(self.fsdp_module)
            ...
            continuation_ids_list = self.rollout.generate_continuations(
                prefix_ids_list=prefix_ids_list,
                multi_modal_data_list=multi_modal_data,
                max_new_tokens=cf_max_tokens,
                temperature=0.0,
            )
        if len(continuation_ids_list) != batch_size:
            print(
                f"[CIT-Faith] Expected {batch_size} continuations, got {len(continuation_ids_list)}. "
                "Padding or truncating continuation outputs."
            )
            continuation_ids_list = continuation_ids_list[:batch_size]
            continuation_ids_list.extend([[] for _ in range(batch_size - len(continuation_ids_list))])

        # Pad continuations to uniform length
        max_requested_tokens = max(int(cf_max_tokens), 1)
        max_cont_len = max((len(c) for c in continuation_ids_list), default=0)
        max_cont_len = max(1, min(max_cont_len, max_requested_tokens))
        pad_token_id = self._pad_token_id()
        padded_cont = torch.full((batch_size, max_cont_len), pad_token_id, dtype=torch.long)
        cont_mask = torch.zeros(batch_size, max_cont_len, dtype=torch.long)

        for i, cont_ids in enumerate(continuation_ids_list):
            length = min(len(cont_ids), max_cont_len)
            padded_cont[i, :length] = torch.tensor(cont_ids[:length], dtype=torch.long)
            cont_mask[i, :length] = 1

        output = DataProto.from_dict(
            tensors={
                "continuation_ids": padded_cont,
                "continuation_mask": cont_mask,
            }
        )
        return output
