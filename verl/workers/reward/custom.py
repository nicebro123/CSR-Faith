        ...
        for i in range(len(data)):
            ...
            try:
                if self.config.score_function == "spatial_sgg":
                    if "problem" not in data_item.non_tensor_batch:
                        raise KeyError("spatial_sgg reward requires `problem` in non_tensor_batch.")
                    problem = data_item.non_tensor_batch["problem"]
                    score = self.compute_score(response_str, ground_truth, problem)
                else:
                    score = self.compute_score(response_str, ground_truth)
            except Exception as exc:
                print(f"[Reward] Failed to score sample {i}: {exc}")
                reward_metrics["overall"].append(0.0)
                reward_metrics["error"].append(1.0)
                continue

            reward_tensor[i, valid_response_length - 1] = float(score.get("overall", 0.0))
            for key, value in score.items():
                reward_metrics[key].append(value)
            reward_metrics["error"].append(0.0)
